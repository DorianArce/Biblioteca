/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/GUIForms/JPanel.java to edit this template
 */
package com.mycompany.views;

import com.mycompany.biblioteca.DAOPrestamosIMPL;
import com.mycompany.interfaces.DAOPrestamos;
import com.mycompany.utils.Utils;

/**
 *
 * @author doria
 */
public class Devoluciones extends javax.swing.JPanel {

    /**
     * Creates new form Bienvenida
     */
    public Devoluciones() {
        initComponents();
        initStyles();
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jPanel1 = new javax.swing.JPanel();
        jLabel1 = new javax.swing.JLabel();
        NPedido = new javax.swing.JTextField();
        Devolver = new javax.swing.JButton();
        FFecha = new javax.swing.JTextField();
        jLabel3 = new javax.swing.JLabel();
        jLabel4 = new javax.swing.JLabel();

        setPreferredSize(new java.awt.Dimension(668, 375));
        setRequestFocusEnabled(false);

        jPanel1.setPreferredSize(new java.awt.Dimension(668, 375));

        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Prestamos.png"))); // NOI18N
        jLabel1.setText("Devoluciones");

        Devolver.setBackground(new java.awt.Color(0, 102, 102));
        Devolver.setFont(new java.awt.Font("Segoe UI", 1, 18)); // NOI18N
        Devolver.setForeground(new java.awt.Color(255, 255, 255));
        Devolver.setText("Devolver");
        Devolver.setBorderPainted(false);
        Devolver.setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
        Devolver.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                DevolverActionPerformed(evt);
            }
        });

        jLabel3.setText("Numero de pedido");

        jLabel4.setText("Fecha de devolucion");

        javax.swing.GroupLayout jPanel1Layout = new javax.swing.GroupLayout(jPanel1);
        jPanel1.setLayout(jPanel1Layout);
        jPanel1Layout.setHorizontalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(FFecha, javax.swing.GroupLayout.Alignment.TRAILING)
                    .addComponent(jLabel1, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGap(25, 25, 25)
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(jLabel4, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addComponent(jLabel3, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addGroup(jPanel1Layout.createSequentialGroup()
                                .addComponent(Devolver, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addGap(683, 683, 683))))
                    .addComponent(NPedido))
                .addContainerGap())
        );
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jLabel1, javax.swing.GroupLayout.PREFERRED_SIZE, 70, Short.MAX_VALUE)
                .addGap(48, 48, 48)
                .addComponent(jLabel3, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(NPedido, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(49, 49, 49)
                .addComponent(jLabel4, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(FFecha, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(32, 32, 32)
                .addComponent(Devolver, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(257, 257, 257))
        );

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(this);
        this.setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(jPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 820, Short.MAX_VALUE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(jPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 626, Short.MAX_VALUE)
        );
    }// </editor-fold>//GEN-END:initComponents

    private void DevolverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_DevolverActionPerformed
        String npedido = NPedido.getText();
        String FechaE = FFecha.getText();
        if (npedido.isEmpty() || FechaE.isEmpty()) {
            javax.swing.JOptionPane.showMessageDialog(this, "Debe llenar todos los campos. \n", "AVISO", javax.swing.JOptionPane.ERROR_MESSAGE);
            NPedido.requestFocus();
            return;
        }
        
        else if (!Utils.isNumeric(npedido) ) {
            javax.swing.JOptionPane.showMessageDialog(this, "Los campos Folio y el ID del libro deben ser números enteros. \n", "AVISO", javax.swing.JOptionPane.ERROR_MESSAGE);
            NPedido.requestFocus();
            return;
        }
        com.mycompany.models.Prestamos prestamo = new com.mycompany.models.Prestamos();
        prestamo.setNum_Pedido(npedido);
        prestamo.setFecha_Devolucion(FechaE);
        try{
            DAOPrestamos daoUsers = new DAOPrestamosIMPL();
            com.mycompany.models.Prestamos currentPrest = daoUsers.getPrestamoById(Integer.parseInt(npedido));
            if (currentPrest == null) {
                javax.swing.JOptionPane.showMessageDialog(this, "No se encontró ningún pedido con ese folio. \n", "AVISO", javax.swing.JOptionPane.ERROR_MESSAGE);
                NPedido.requestFocus();
                return;
            }
            
            DAOPrestamos dao = new DAOPrestamosIMPL();
            dao.modificar(prestamo);
            javax.swing.JOptionPane.showMessageDialog(this, "Devolucion exitosa.\n", "AVISO", javax.swing.JOptionPane.INFORMATION_MESSAGE);
        }catch(Exception e){
            javax.swing.JOptionPane.showMessageDialog(this, "Ocurrió un error al devolver el libro. \n", "AVISO", javax.swing.JOptionPane.ERROR_MESSAGE);
            System.out.println(e.getMessage());
        }
    }//GEN-LAST:event_DevolverActionPerformed


    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton Devolver;
    private javax.swing.JTextField FFecha;
    private javax.swing.JTextField NPedido;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JLabel jLabel3;
    private javax.swing.JLabel jLabel4;
    private javax.swing.JPanel jPanel1;
    // End of variables declaration//GEN-END:variables

    private void initStyles() {
        jLabel1.putClientProperty("FlatLaf.styleClass", "h0");
        jLabel3.putClientProperty("FlatLaf.styleClass", "large");
        jLabel4.putClientProperty("FlatLaf.styleClass", "large");
        NPedido.putClientProperty("JTextField.placeholderText", "Ingrese el numero de pedido");
        FFecha.putClientProperty("JTextField.placeholderText", "Ingrese la fecha en el formato AAAA-MM-DD");
    }
}
